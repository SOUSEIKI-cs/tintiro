...
export default function GameTintiro({gamestartbool,membernum,setgameend,gameend,setwinner,playerhp,setplayerhp}:{gamestartbool:boolean,membernum:number,setgameend:React.Dispatch<SetStateAction<boolean>>,gameend:boolean,setwinner:React.Dispatch<SetStateAction<number>>,playerhp:Array<number>,setplayerhp:React.Dispatch<SetStateAction<Array<number>>>}){
  ...
  function nextPlayer(){
    ...
    setnowplayer(()=>{
      let nextnotzeroplayer = playerhp.findIndex((element,index)=>element>0&&index>nowplayer);
      const lastzeroplayer = playerhp.findLastIndex(element => element > 0);
      if((nowplayer===lastzeroplayer)){
        setendattack(false);
        nextnotzeroplayer = playerhp.findIndex((element,index)=>element>0&&index>=0);
        return nextnotzeroplayer;
      }
      return nextnotzeroplayer;
    })
  };
  ...
  function directattack(){
    ...
    setendattack(true);
    const zerohpplayer= playerhpbuffer.filter(num=>num<=0);
    const maxhp= Math.max(...playerhpbuffer);
    setwinner(playerhpbuffer.indexOf(maxhp)+1);
    setgameend((zerohpplayer.length) >= 9);
    const nextnotzeroplayer = playerhpbuffer.findIndex((element,index)=>element>=500&&index>=0);
    setnowplayer(nextnotzeroplayer);
  };
...
function DirectAttackBtn({nowplayer,playerhp,directattack,endattack}:{nowplayer:number,playerhp:Array<number>,directattack:React.MouseEventHandler<HTMLButtonElement>,endattack:boolean}){
  const nextnotzeroplayer = playerhp.findIndex((element)=>element>0);
  console.log(endattack);
  if(nowplayer===nextnotzeroplayer&&!endattack){
    return(
